...
class PolicyGraph():
    ...
    def __init__(self, input_states, taken_actions,
        ...
        with tf.variable_scope(scope_name):
            # Construct model
            self.conv1 = tf.layers.conv2d(input_states, filters=24, kernel_size=5,
                                          strides=2, activation=tf.nn.leaky_relu, padding="valid", name="conv1")
            self.conv2 = tf.layers.conv2d(self.conv1, filters=48, kernel_size=3,
                                          strides=1, activation=tf.nn.leaky_relu, padding="valid", name="conv2")
            self.shared_features = tf.layers.flatten(self.conv2)
